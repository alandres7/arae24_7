...
import co.gov.metropol.area247.covid19.dto.CoordenadaDto;
...
public class GeometryUtil {
	 ...
	 * 
	 * @param coordenada - objeto que contiene el valor de la latitud y longitud
	 * 
	 ...
	 * 
	 * @param latitud  - valor del eje x del punto
	 * @param longitud - valor del eje y del punto
	 * 
	 ...
	 * 
	 * @param coordenadas - contiene los puntos geometricos
	 * 
	...
	public static Polygon createPolygon(final List<CoordenadaDto> outerRing,
			final List<List<CoordenadaDto>> innersRings) {
		Coordinate[] coordinates = createListCoordinate(outerRing);
		LinearRing shell = createLinearRing(coordinates);
		LinearRing[] holes = createLinearRings(innersRings);
		Polygon pPolygon = FACTORIA.createPolygon(shell, holes);
		return pPolygon;

	}

	public static LinearRing[] createLinearRings(final List<List<CoordenadaDto>> innersRings) {
		LinearRing[] holes = new LinearRing[innersRings.size()];
		for (int i = 0; i < innersRings.size(); i++) {
			List<CoordenadaDto> coordenadasDto = innersRings.get(i);
			holes[i] = createLinearRing(createListCoordinate(coordenadasDto));
		}
		return holes;
	}

	public static LinearRing createLinearRing(final Coordinate[] listCoordinates) {
		LinearRing shell = FACTORIA.createLinearRing(listCoordinates);
		return shell;
	}

	public static Coordinate[] createListCoordinate(final List<CoordenadaDto> coordenadasDto) {
		final Coordinate[] puntos = new Coordinate[coordenadasDto.size()];
		for (int i = 0; i < coordenadasDto.size(); i++) {
			Coordinate punto = new Coordinate(coordenadasDto.get(i).getLongitud(), coordenadasDto.get(i).getLatitud());
			puntos[i] = punto;
	...
	public static List<CoordenadaDto> conversorCoordenadaDto(LineString lineString) {
		...
		try {
			coordenadaDto = (ArrayList<CoordenadaDto>) Arrays.asList(lineString.getCoordinates()).stream()
					.filter(Objects::nonNull).map(c -> new CoordenadaDto(c.y, c.x)).collect(Collectors.toList());
		} catch (Exception e) {
	...
	public static List<String> conversorCoordenada(LineString lineString) {
		...
		try {
			Arrays.asList(lineString.getCoordinates()).stream().filter(Objects::nonNull)
					.map(c -> new CoordenadaDto(c.y, c.x)).forEach(e -> {
	...
	public static List<List<LatLng>> getCoordenadasMaps(Polygon polygon) {
		List<List<LatLng>> coordenadasMaps = new ArrayList<>();
		coordenadasMaps.add(Arrays.asList(polygon.getExteriorRing().getCoordinates()).stream().filter(Objects::nonNull)
				.map(c -> new LatLng(c.y, c.x)).collect(Collectors.toList()));
		int i = 0;
		while (i < polygon.getNumInteriorRing()) {
			List<LatLng> coordinates = Arrays.asList(polygon.getInteriorRingN(i++).getCoordinates()).stream()
					.filter(Objects::nonNull).map(c -> new LatLng(c.y, c.x)).collect(Collectors.toList());
			Collections.reverse(coordinates);
			coordenadasMaps.add(coordinates);
		}
